...
#if defined(NDEBUG)
# error "Ramsikka cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8f3abfa0e200e5311fca63fd82abaaf27cab718cb9ca8fbd4870c5589485ff94");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ramsikka: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ramsikka Signed Message:\n";
...
{
    // Ramsikka: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 13000); // Ramsikka: 13k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Ramsikka: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Ramsikka: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ramsikka: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ramsikka: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 0xbu;
        pchMessageStart[3] = 0xd1;
        hashGenesisBlock = uint256("0xf62dfce2a9a5572def17ac0ac21879d6f17c827f6788bc2719c95165959a931d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Luna dies and rissen from its fallen ashes, 16 May, 2022";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1652694315;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084796749;

        if (fTestNet)
        {
            block.nTime    = 1652694304;
            block.nNonce   = 385708362;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcb46fe68cc52dfc143aefdc4e3bf6027e7b4b9191cb8985068bd2be8503191ee"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc6, 0xbu, 0xd7 }; // Ramsikka: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RamsikkaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RamsikkaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RamsikkaMiner(CWallet *pwallet)
{
    printf("RamsikkaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ramsikka-miner");
    ...
    try { loop {
        ...
        printf("Running RamsikkaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RamsikkaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RamsikkaMiner, pwallet));
}
